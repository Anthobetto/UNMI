...
export class FlowService {
    ...
    async handleMissedCall(event) {
        ...
        if (preferences.preferredFlow === 'chatbot' || preferences.preferredFlow === 'both') {
            ...
            if (preferences.autoActivateChatbot && preferences.defaultChatbotId) {
                try {
                    const result = await this.autoRouteToChatbot(preferences.defaultChatbotId, event.userId, `Missed call from ${event.callerNumber}`);
                    if (result.success) {
    ...
    async autoCompleteAndSendTemplate(completion) {
        ...
        const result = await providerService.sendWhatsApp(completion.recipientNumber, templateMessage);
        if (result.success) {
    ...
    async handleMissedCallWithWhatsApp(callData) {
        try {
            ...
            const locationTemplates = templates.filter(t => t.locationId === callData.locationId &&
                t.channel === 'whatsapp' &&
                t.type === 'missed_call');
