  ...
  return (
    <>
      ...
      <div className="space-y-6 mt-1 pb-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div 
            ...
            className={`p-6 rounded-[2rem] border-2 cursor-pointer transition-all flex flex-col ${
              selectedPlanUI === 'small' ? 'border-[#FF0000] bg-white shadow-xl' : 'border-gray-100 bg-gray-50/50'
            }`}
          >
          ...
          <div 
            ...
            className={`relative p-6 rounded-[2rem] border-2 cursor-pointer transition-all flex flex-col ${
              selectedPlanUI === 'pro' ? 'border-[#FF0000] bg-white shadow-xl' : 'border-gray-100 bg-gray-50/50'
            }`}
          >
            <div className="flex items-center gap-3 mb-4">
          ...
          <div 
            ...
            className={`p-6 rounded-[2rem] border-2 cursor-pointer transition-all flex flex-col ${
              selectedPlanUI === 'premium' ? 'border-[#FF0000] bg-white shadow-xl' : 'border-gray-100 bg-gray-50/50'
            }`}
          >
            <div className="flex items-center gap-3 mb-4">
        ...
        <div className="bg-[#0A0A0A] text-white p-8 rounded-[2.5rem] flex flex-col md:flex-row justify-between items-center shadow-2xl">
          <div className="mb-6 md:mb-0">
            <p className="text-red-500 text-xs font-black uppercase tracking-[0.2em] mb-2">Resumen de actualización</p>
            <h4 className="text-xl font-bold">
              {user?.planType === selectedPlanUI ? 'Mantener' : 'Cambiar a'} {selectedPlanUI.toUpperCase()} 
              <span className="text-gray-500 ml-2">({numLocations} sedes / {numDepartments} depts)</span>
            </h4>
          </div>
          <div className="flex items-center gap-8">
            <div className="text-right">
               <p className="text-4xl font-black">€{calculateTotal()}</p>
               <p className="text-gray-500 text-xs uppercase font-bold tracking-tighter">Total mensual (IVA incl.)</p>
            </div>
            <Button 
              onClick={handleUpgrade} 
              disabled={isProcessing}
              className="bg-[#FF0000] hover:bg-[#cc0000] h-16 px-10 rounded-2xl text-xl font-black shadow-xl shadow-red-500/20 transition-all active:scale-95"
            >
